...
function isString(value) {
  return (
    Object.prototype.toString.call(value).slice(0, -1).split(' ')[1] ===
    'String'
  );
}
...
function getFirstChar(value) {
  if (value[0]) return value.charAt(0);
  return value;
}
...
function sumOfCodes(str) {
  if (str) return str.split('').reduce((acc, e) => acc + e.charCodeAt(), 0);
  return 0;
}
...
function countVowels(str) {
  return str.toLowerCase().replace(/[^aeiouy]/gi, '').length;
}
...
function isPalindrome(str) {
  ...
  if (tempStr.length <= 1) return true;
  if (tempStr[0] === tempStr[tempStr.length - 1])
    return isPalindrome(tempStr.slice(1, tempStr.length - 1));
  return false;
}
...
function findLongestWord(sentence) {
  const arrOfWords = sentence.split(' ').reverse();
  return arrOfWords.reduce((acc, wrd) => (acc.length > wrd.length ? acc : wrd));
}
